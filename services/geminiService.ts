...
export const translateText = async (text: string, direction: 'vi_en' | 'en_vi' = 'vi_en'): Promise<TranslationResponse> => {
  try {
    ...
    const isApiKeyError = msg.includes("API key") || msg.includes("400") || msg.includes("403") || msg.includes("PERMISSION_DENIED");
    const isQuotaError = msg.includes("429") || msg.includes("quota") || msg.includes("RESOURCE_EXHAUSTED");
    
    let userMsg = "Connection Failed";
    if (isApiKeyError) userMsg = "Invalid API Key";
    else if (isQuotaError) userMsg = "Quota Exceeded (Try later)";
    else userMsg += `: ${msg.substring(0, 30)}...`;

    return { 
      english: `Error: ${userMsg}`, 
      phonetic: "",
      partOfSpeech: "System", 
      usageHint: isApiKeyError ? "Please check your API Key in Settings." : "Check network or try again."
    };
  }
};
